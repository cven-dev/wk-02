...
Your main objectives are to: - Run each code chunk and ensure it executes without errors - Verify that each step produces the expected results - Understand the connection between the task descriptions and the code implementation
...
2.  Change the code-chunk option `#| eval:` from `false` to `true`

3.  Run the code contained in the code-chunk and verify it executes without errors
...
-   Five variables are selected: country_name, iso3c, income_id, total_msw_total_msw_generated_tons_year, and population_population_number_of_people
...
5.  Describe in your own words what the `mutate()` function does in this code-chunk
...
5.  Look at the plot that you created and describe what the `mutate()` function achieved in the previous task

<!--# Remove this comment and write your text here -->

6.  Describe in your own words what the 'color' argument inside the `aes()` does

<!--# Remove this comment and write your text here -->

7.  Describe in your own words what the `labs()` function does
